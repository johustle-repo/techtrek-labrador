...
export default function Welcome() {
    ...
    return (
        <>
            ...
            <div className="min-h-screen bg-slate-50 font-['Instrument_Sans'] text-slate-900 selection:bg-teal-500 selection:text-white">
                <header className="relative overflow-hidden pb-30 pt-20 md:pb-36 md:pt-28">
                    <div className="absolute inset-0">
                        ...
                        />
                        <div className="absolute inset-0 bg-gradient-to-b from-emerald-950/84 via-emerald-900/50 to-slate-50/96" />
                        <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_10%,rgba(20,184,166,0.25),transparent_40%),radial-gradient(circle_at_82%_18%,rgba(16,185,129,0.2),transparent_38%)]" />
                        ...
                        <div className="space-y-6">
                            ...
                            <div>
                                <a
                                    ...
                                    className="inline-flex rounded-full bg-white px-7 py-3 text-sm font-bold text-slate-900 shadow-xl shadow-black/20 transition hover:scale-[1.02]"
                                >
                                    Get Started Now
                                </a>
                            </div>
                    ...
                    <div className="mx-auto max-w-7xl space-y-12">
                        <section id="explore" className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
                            {featureCards.map((item) => (
                                <Link
                                    ...
                                    prefetch
                                    className="group relative overflow-hidden rounded-3xl border border-white/70 bg-white p-7 shadow-lg shadow-slate-200/60 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-2xl"
                                >
                        ...
                        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-lg shadow-slate-200/50">
                            <div className="mb-5 flex items-end justify-between gap-4">
                                <div>
                                    <p className="text-xs font-bold uppercase tracking-[0.12em] text-teal-700">Incoming Events</p>
                                    <h2 className="text-3xl font-extrabold tracking-tight text-slate-900">Upcoming Municipal Activities</h2>
                                </div>
                                <Link href={moduleHref('/events')} prefetch className="text-sm font-bold text-teal-700 transition hover:text-teal-600">
                                    View all events
                                </Link>
                            </div>
                                    ...
                                    {incoming_events.map((event) => (
                                        <article key={event.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
                                            ...
                                            <div className="space-y-1 p-4">
                                                ...
                                                <p className="text-sm text-slate-600">{event.starts_at ? new Date(event.starts_at).toLocaleString() : 'Date to be announced'}</p>
                                                <p className="text-sm text-slate-600">{event.venue_name ?? 'Venue to be announced'}</p>
                                            </div>
                                        </article>
                                    ))}
                                </div>
                        ...
                        <section id="about-labrador" className="scroll-mt-28 grid gap-6 lg:grid-cols-[1.2fr_1fr]">
                            <div className="rounded-3xl border border-slate-200 bg-white p-7 shadow-lg shadow-slate-200/50">
                                <div className="mb-5 flex items-center gap-4 rounded-2xl border border-teal-100 bg-teal-50 px-4 py-3">
                                    <img
                                    ...
                                    />
                                    <div>
                                        <p className="text-xs font-bold uppercase tracking-[0.11em] text-teal-700">Official Seal</p>
                                        <p className="text-sm font-semibold text-slate-800">Municipality of Labrador</p>
                                        <p className="text-xs text-slate-600">Pangasinan, Philippines</p>
                                    </div>
                                </div>

                                <h2 className="mb-3 text-3xl font-extrabold tracking-tight text-slate-900">About Labrador</h2>
                                ...
                                <ul className="space-y-3">
                                    {profileFacts.map((fact) => (
                                        <li key={fact} className="flex items-start gap-2.5 text-sm text-slate-700">
                            ...
                            <div className="rounded-3xl border border-slate-200 bg-white p-7 shadow-lg shadow-slate-200/50">
                                <h2 className="mb-3 text-3xl font-extrabold tracking-tight text-slate-900">Local Products</h2>
                                ...
                                <div className="flex flex-wrap gap-2.5">
                                    {products.map((item) => (
                                        <span key={item} className="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-xs font-semibold text-emerald-800">
                                            {item}
                                        </span>
                                    ))}
                                </div>
                        ...
                        <section>
                            <div className="mb-5 flex items-end justify-between gap-4">
                                <div>
                                    <p className="text-xs font-bold uppercase tracking-[0.12em] text-teal-700">Top Destinations</p>
                                    <h2 className="text-3xl font-extrabold tracking-tight text-slate-900">Must-Visit Places in Labrador</h2>
                                </div>
                            </div>
                                ...
                                <div className="grid gap-6 md:grid-cols-3">
                                    {must_visit_attractions.map((item) => (
                                        <Link
                                            ...
                                            prefetch
                                            className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-lg shadow-slate-200/60 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
                                        >
