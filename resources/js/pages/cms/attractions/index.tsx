...
import { Clock3, ImageOff, Layers3, MapPin, Plus } from 'lucide-react';
...
function statusVariant(status: AttractionRow['status']) {
    if (status === 'published') return 'default';
    if (status === 'archived') return 'secondary';
    return 'outline';
}

export default function AttractionsIndex({ attractions }: Props) {
    const page = usePage<Props>();
    const success = page.props.flash?.success;
    const totalRecords = attractions.data.length;

    const handleDelete = (id: number) => {
        if (!confirm('Delete this attraction? This action cannot be undone.')) {
            return;
        }

        router.delete(`/cms/attractions/${id}`);
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="CMS Attractions" />
            ...
            <div className="flex flex-1 flex-col gap-6 p-4 md:p-6">
                    ...
                    <div className="relative flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                        <div className="max-w-2xl">
                            ...
                            <p className="mt-1 text-sm text-muted-foreground">
                                Manage Labrador tourist attraction entries with
                        ...
                        <div className="flex w-full items-start justify-start gap-3 md:w-auto md:justify-end">
                            ...
                            <Button asChild className="h-11 px-5">
                                <Link href="/cms/attractions/create">
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>Attraction Records</CardTitle>
                    </CardHeader>
                        ...
                        {attractions.data.length === 0 && (
                            <div className="rounded-xl border border-dashed p-10 text-center">
                                ...
                                <p className="mt-3 text-sm text-muted-foreground">
                                    No attractions yet. Add your first entry to
                                    ...
                                    <div className="space-y-3 p-4">
                                        ...
                                        <div className="flex gap-2 pt-1">
                                            <Button variant="outline" asChild>
                                                <Link
                                                    href={`/cms/attractions/${item.id}/edit`}
                                                >
                                                    Edit
                                                </Link>
                                            </Button>
